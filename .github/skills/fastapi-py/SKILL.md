...
| **Advanced** | [advanced-concepts.md](references/advanced-concepts.md) | Lifespan, middleware, WebSockets, streaming, caching, GraphQL |
...
- Use `# pyright: ignore[reportUnusedFunction]` for route handlers in factory functions (basedpyright false positive)

## Anti-Patterns to Avoid

- ❌ Business logic in route handlers (use services)
- ❌ Raw SQL in routes (use repositories)
- ❌ Sync I/O in async functions (blocks event loop)
- ❌ Hardcoded secrets (use pydantic-settings + env vars)
- ❌ Missing `response_model` (leaks internal fields)
- ❌ Catching bare `Exception` in routes (use specific exceptions)
